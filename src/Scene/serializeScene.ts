 ...
 * scene's file. Entities tagged `Transient` (the gizmo's handle entities -
 * see GizmoAxis.ts - regenerated by App every frame from the current
 * selection) are skipped entirely, the same way an unrelated asset is never
 * leaked in: they're runtime editor state, not something a save should ever
 * capture.
 *
...
export function serializeScene(
	...
	const entities: EntityJSON[] = world
		.entities()
		.filter((entity) => !world.has(entity, Transient))
		.map((entity) => {
	...
	const materialAssets: AssetJSON[] = Array.from(usedMaterialIds).map(
		(id) => {
